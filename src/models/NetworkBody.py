...
import sys 
sys.path.append('/workspace/unsupFlownet/src')
print(sys.path)
...
class NetworkBody:
	...
	def __init__(self,trainingData,instanceParams,flipInput=False):
		frame0 = trainingData[0] # c, h, w
		frame1 = trainingData[1] # c, h, w

		if flipInput:
			combined = torch.cat([frame1, frame0], 0)
		else:
			combined = torch.cat([frame0, frame1], 0)
